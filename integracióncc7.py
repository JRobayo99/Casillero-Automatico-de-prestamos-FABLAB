...
class IdentificadorCedulas:
    ...
    def configurar_camara(self):
        """Configura la cámara con la resolución deseada - OPTIMIZADA PARA VELOCIDAD"""
        ...
        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Reducir buffer
        self.cap.set(cv2.CAP_PROP_FPS, 30)  # Intentar 30 FPS
    ...
    def mostrar_pantalla_carga(self, tipo_cedula):
        ...
        # Obtener dimensiones de pantalla para escalado
        screen_width = root.winfo_screenwidth()
        screen_height = root.winfo_screenheight()
        
        # Calcular tamaños de fuente basados en altura de pantalla
        fuente_titulo = int(screen_height * 0.03)  # 3% de la altura
        fuente_grande = int(screen_height * 0.08)  # 8% de la altura
        fuente_mediana = int(screen_height * 0.05)  # 5% de la altura
        fuente_pequena = int(screen_height * 0.025)  # 2.5% de la altura
        ...
        # Configurar grid con pesos para centrar verticalmente
        ...
        # Título del tipo de cédula detectada
        titulo = tk.Label(main_frame,
                          text=f"CÉDULA {tipo_cedula.upper()} DETECTADA",
                          font=('Arial', fuente_titulo, 'bold'),
                          bg='#2E2E2E',
                          fg='white')
        titulo.grid(row=1, column=0, pady=screen_height * 0.02)

        # Subtítulo de carga
        subtitulo = tk.Label(main_frame,
                             text="Cargando escáner...",
                             font=('Arial', fuente_mediana, 'bold'),
                             bg='#2E2E2E',
                             fg='#CCCCCC')
        subtitulo.grid(row=2, column=0, pady=screen_height * 0.02)
        ...
        cuenta_regresiva = tk.Label(main_frame,
                                    ...
                                    bg='#2E2E2E',
                                    fg='#FFD700')
        cuenta_regresiva.grid(row=3, column=0, pady=screen_height * 0.03)

        # Número grande para la cuenta regresiva
        tiempo_label = tk.Label(main_frame,
                                text="5",
                                font=('Arial', fuente_grande, 'bold'),
                                bg='#2E2E2E',
                                fg='#FFD700')
        tiempo_label.grid(row=4, column=0, pady=screen_height * 0.05)

        # Instrucción
        instruccion = tk.Label(main_frame,
                                text="Por favor, espere mientras se carga el sistema",
                                font=('Arial', fuente_pequena),
                                bg='#2E2E2E',
                                fg='#AAAAAA')
        instruccion.grid(row=5, column=0, pady=screen_height * 0.02)
    ...
    def iniciar_identificacion(self):
        """Inicia el proceso de identificación con mejor tasa de frames"""
        ...
        # REDUCIR FRECUENCIA DE ESCANEO para mejor rendimiento
        scan_counter = 0
        SCAN_EVERY_N_FRAMES = 3  # Escanear cada 3 frames
        ...
        while not self.detection_complete:
            ...
            # Dibujar ROI de identificación (OPERACIÓN RÁPIDA)
            cv2.rectangle(frame, 
                         (self.roi_ident_x1, self.roi_ident_y1), 
                         (self.roi_ident_x2, self.roi_ident_y2), 
                         (0, 255, 0), 3)
            
            # Texto informativo (OPERACIÓN RÁPIDA)
            cv2.putText(frame, "COLOQUE LA CEDULA AQUI", 
                       (self.roi_ident_x1, self.roi_ident_y1 - 20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            
            cv2.putText(frame, f"IDENTIFICANDO... FPS: {current_fps}", 
                       (50, 50), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
            
            # ESCANEO AUTOMÁTICO - SOLO CADA N FRAMES
            scan_counter += 1
            if scan_counter >= SCAN_EVERY_N_FRAMES:
                scan_counter = 0
                ...
                try:
                    ...
                    # Solo procesar si hay contenido en el ROI
                    if roi.size > 0 and np.mean(roi) > 30:
...
class EscanerCedulaAntigua:
    ...
    def configurar_camara(self):
        """Configura la cámara - OPTIMIZADA PARA VELOCIDAD"""
        self.cap = cv2.VideoCapture(0)
    ...
    def mostrar_resultados_tkinter(self, data):
        """Muestra los resultados en una ventana de tkinter con botones escalados"""
        self.resultado_window = tk.Tk()
        self.resultado_window.title("Resultado de Escaneo - Cédula Antigua")
        self.resultado_window.attributes('-fullscreen', True)
        
        # Obtener dimensiones de pantalla para escalado
        screen_width = self.resultado_window.winfo_screenwidth()
        screen_height = self.resultado_window.winfo_screenheight()
        
        # Calcular tamaños proporcionales a la pantalla
        padding = int(screen_width * 0.03)  # 3% del ancho
        fuente_bienvenida = int(screen_height * 0.05)  # 5% de la altura
        fuente_titulo_datos = int(screen_height * 0.035)  # 3.5% de la altura
        fuente_datos = int(screen_height * 0.025)  # 2.5% de la altura
        fuente_botones = int(screen_height * 0.03)  # 3% de la altura
        tamanio_boton = int(screen_height * 0.06)  # 6% de la altura para el alto del botón
        
        self.resultado_window.configure(bg='#2E2E2E')
        
        # Frame principal
        main_frame = tk.Frame(self.resultado_window, bg='#2E2E2E')
        main_frame.pack(expand=True, fill='both', padx=padding, pady=padding)
        
        # Título de bienvenida
        nombre_completo = f"{data.get('Nombre', '')} {data.get('Primer apellido', '')} {data.get('Segundo apellido', '')}".strip()
        if not nombre_completo:
            nombre_completo = "Usuario"
            
        bienvenida = tk.Label(main_frame, 
                              text=f"¡Bienvenido/a {nombre_completo}!", 
                              font=('Arial', fuente_bienvenida, 'bold'),
                              bg='#2E2E2E',
                              fg='#FFD700',
                              wraplength=screen_width * 0.9)  # Wrap text if too long
        bienvenida.pack(pady=screen_height * 0.05)
        
        # Frame para los datos
        datos_frame = tk.Frame(main_frame, bg='#404040', relief='ridge', bd=3)
        datos_frame.pack(expand=True, fill='both', pady=screen_height * 0.05)
        
        # Título de datos
        titulo_datos = tk.Label(datos_frame, 
                                text="DATOS DE LA CÉDULA", 
                                font=('Arial', fuente_titulo_datos, 'bold'),
                                bg='#404040',
                                fg='white')
        titulo_datos.pack(pady=screen_height * 0.03)
        
        # Mostrar cada dato
        for key, value in data.items():
            if value:  # Solo mostrar si tiene valor
                frame_dato = tk.Frame(datos_frame, bg='#404040')
                frame_dato.pack(fill='x', padx=padding, pady=screen_height * 0.01)
                
                label_key = tk.Label(frame_dato, 
                                    text=f"{key}:", 
                                    font=('Arial', fuente_datos, 'bold'),
                                    bg='#404040',
                                    fg='#FFD700',
                                    width=int(screen_width * 0.02),  # Ancho proporcional
                                    anchor='w')
                label_key.pack(side='left')
                
                label_value = tk.Label(frame_dato, 
                                      text=str(value), 
                                      font=('Arial', fuente_datos),
                                      bg='#404040',
                                      fg='white',
                                      anchor='w',
                                      wraplength=screen_width * 0.5)  # Wrap long values
                label_value.pack(side='left', padx=padding)
        
        # Frame para botones
        botones_frame = tk.Frame(main_frame, bg='#2E2E2E')
        botones_frame.pack(pady=screen_height * 0.05)
        
        def volver_a_escanear():
        ...
        def continuar():
            self.resultado_window.destroy()
            print("\n✅ Proceso completado. Saliendo del sistema...")
        
        # Botón Volver a escanear
        btn_volver = tk.Button(botones_frame, 
                               text="VOLVER A ESCANEAR", 
                               command=volver_a_escanear,
                               font=('Arial', fuente_botones, 'bold'),
                               bg='#4CAF50',
                               fg='white',
                               width=int(screen_width * 0.015),  # Ancho proporcional
                               height=int(tamanio_boton / fuente_botones),  # Alto proporcional
                               cursor='hand2')
        btn_volver.pack(side='left', padx=padding)
        
        # Botón Continuar
        btn_continuar = tk.Button(botones_frame, 
                                  text="CONTINUAR", 
                                  command=continuar,
                                  font=('Arial', fuente_botones, 'bold'),
                                  bg='#2196F3',
                                  ...
                                  height=int(tamanio_boton / fuente_botones),
                                  cursor='hand2')
        btn_continuar.pack(side='left', padx=padding)
        
        # Instrucción para salir
        instruccion = tk.Label(main_frame,
                               text="Presione ESC para salir",
                               font=('Arial', int(fuente_datos * 0.7)),
                               bg='#2E2E2E',
                               fg='#AAAAAA')
        instruccion.pack()
        
        # Bind tecla ESC para salir
        self.resultado_window.bind('<Escape>', lambda e: continuar())
        
        self.resultado_window.mainloop()
    
    def iniciar_escaneo(self):
        """Inicia el escaneo específico para cédula antigua con mejor tasa de frames"""
        self.configurar_camara()
        ...
        scan_counter = 0
        SCAN_EVERY_N_FRAMES = 2  # Escanear cada 2 frames
        ...
        while not self.detected:
            ...
            # Dibujar ROI específico (OPERACIÓN RÁPIDA)
            cv2.rectangle(frame, (self.x1, self.y1), (self.x2, self.y2), (0, 255, 0), 2)
            cv2.putText(frame, f"COLOQUE EL CODIGO DE BARRAS AQUI - FPS: {current_fps}", 
            ...
            if scan_counter >= SCAN_EVERY_N_FRAMES:
                ...
                if cropped.size > 0:
                    try:
                        # USAR IMAGEN REDIMENSIONADA PARA ESCANEO MÁS RÁPIDO
                        ...
                        if width > 640:
                            ...
                            cropped_small = cv2.resize(cropped, (new_width, new_height))
                        else:
                            cropped_small = cropped
                        
                        cropped_rgb = cv2.cvtColor(cropped_small, cv2.COLOR_BGR2RGB)
                        ...
                        if results:
                            for r in results:
                                ...
                                if extracted.get("Cédula") and extracted.get("Primer apellido"):
                                    ...
                                    self.cap.release()
                                    cv2.destroyAllWindows()
                                    self.mostrar_resultados_tkinter(extracted)
                                    break
                    except Exception as e:
                        print(f"Error: {e}")
    ...
    def mostrar_resultados(self, data):
        """Muestra los resultados del escaneo (versión consola)"""
        print("\n" + "="*60)
...
class EscanerCedulaNueva:
    ...
    def mostrar_resultados_tkinter(self, datos):
        """Muestra los resultados en una ventana de tkinter con botones escalados"""
        self.resultado_window = tk.Tk()
        self.resultado_window.title("Resultado de Escaneo - Cédula Nueva")
        self.resultado_window.attributes('-fullscreen', True)
        ...
        # Título de bienvenida
        nombre_completo = f"{datos.get('nombres', '')} {datos.get('primer_apellido', '')} {datos.get('segundo_apellido', '')}".strip()
        if not nombre_completo:
            nombre_completo = "Usuario"
        ...
        for key, etiqueta in etiquetas.items():
            if datos.get(key):  # Solo mostrar si tiene valor
                frame_dato = tk.Frame(datos_frame, bg='#404040')
                frame_dato.pack(fill='x', padx=padding, pady=screen_height * 0.01)
    ...
    def iniciar_escaneo(self):
        """Inicia el escaneo específico para cédula nueva con mejor tasa de frames"""
        self.configurar_camara()
        self.configurar_ventana()
        ...
        # REDUCIR FRECUENCIA DE ESCANEO
        scan_counter = 0
        SCAN_EVERY_N_FRAMES = 3  # Escanear cada 3 frames (OCR es lento)
        ...
        while not self.datos_detectados:
            ...
            cv2.putText(frame, f"COLOQUE EL TEXTO DE LA CEDULA AQUI - FPS: {current_fps}", 
            ...
            if scan_counter >= SCAN_EVERY_N_FRAMES:
                ...
                if roi.size > 0:
                    # Verificar si el ROI tiene contenido (no es negro)
                    if np.mean(roi) > 30:
                        # REDIMENSIONAR PARA OCR MÁS RÁPIDO
                        height, width = roi.shape[:2]
                        if width > 800:
                            scale = 800 / width
                            new_width = 800
                            new_height = int(height * scale)
                            roi_small = cv2.resize(roi, (new_width, new_height))
                        else:
                            roi_small = roi
                        
                        # USAR CONFIGURACIÓN MÁS RÁPIDA PARA TESSERACT
                        d = pytesseract.image_to_data(roi_small, lang='spa', 
                        ...
                        if texto_detectado:
                            ...
                            info_texto = f"Caracteres: {len(texto_limpio)}/90 FPS: {current_fps}"
                            ...
                            if len(texto_limpio) >= 90:
                                ...
                                if datos:
                                    ...
                                    self.cap.release()
                                    cv2.destroyAllWindows()
                                    self.mostrar_resultados_tkinter(datos)
                                    break
